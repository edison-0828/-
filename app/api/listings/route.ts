...
function routeError(error: unknown) {
  ...
  if (message.includes("no such table")) {
    return "房源数据表尚未初始化，请先应用 D1 migration。";
  }
  return message;
}
...
export async function POST(request: Request) {
  try {
    ...
    if (leaseEndsAt < availableFrom) {
      return Response.json({ error: "租约到期时间不能早于可入住时间。" }, { status: 400 });
    }
    const db = getDb();
    const [listing] = await db.insert(listings).values({
      id: crypto.randomUUID(),
      publisherId: payload.publisherId || "demo-user",
      title,
