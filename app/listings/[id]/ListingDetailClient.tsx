...
export default function ListingDetailClient({ id }: { id: string }) {
  ...
  useEffect(() => {
    ...
    fetch("/api/listings").then(async (response) => {
      ...
      const index = (payload.listings || []).findIndex((item) => item.id === id);
      if (index < 0) throw new Error("房源不存在或已下架");
      setListing(toListingView(payload.listings![index], index));
  ...
  if (loading) return <main className="zuji-page"><SiteHeader active="find" /><div className="zuji-container zuji-detail-loading">正在加载房源…</div></main>;
  if (!listing || error) return <main className="zuji-page"><SiteHeader active="find" /><div className="zuji-container zuji-detail-loading"><b>{error || "房源不存在"}</b><a href="/">返回房源列表</a></div></main>;

  return <main className="zuji-page"><SiteHeader active="find" userName={userName} /><div className="zuji-container zuji-breadcrumb"><a href="/">找房</a><span>›</span><a href={`/?district=${encodeURIComponent(listing.district)}`}>{listing.district}</a><span>›</span><b>{listing.community}</b></div><section className="zuji-detail-gallery zuji-container"><div className="zuji-detail-main-photo"><img src={listing.images[activeImage]} alt={listing.title} /><span>实拍图片 · {activeImage + 1}/{listing.images.length}</span></div><div className="zuji-detail-thumbs">{listing.images.slice(0, 3).map((image, index) => <button key={image} className={activeImage === index ? "active" : ""} onClick={() => setActiveImage(index)} aria-label={`查看第 ${index + 1} 张房源图片`}><img src={image} alt="" /></button>)}</div></section><section className="zuji-detail-layout zuji-container"><article className="zuji-detail-content"><div className="zuji-detail-title"><div><span>{listing.district} · {listing.community}</span><h1>{listing.title}</h1><p>可入住：{listing.availableFrom}　租约至：{listing.leaseEndsAt || "与发布者确认"}</p></div><button className={saved ? "saved" : ""} aria-label={saved ? "取消收藏" : "收藏房源"} onClick={() => setSaved((value) => !value)}>{saved ? "♥" : "♡"}</button></div><div className="zuji-facts"><div><span>月租金</span><b>¥{listing.price.toLocaleString()}</b></div><div><span>位置</span><b>{listing.district} · {listing.community}</b></div><div><span>入住时间</span><b>{listing.availableFrom}</b></div></div><section className="zuji-detail-section"><h2>关于这套房</h2><p>{listing.description || "房源由真实租客发布。你可以在联系前确认室友、家具、通勤和租期等细节。"}</p></section><section className="zuji-detail-section"><h2>这套房验证了什么</h2><div className="zuji-evidence"><div><span>✓</span><p><b>发布者身份已验证</b><small>确认发布者是本人，但不会公开证件信息。</small></p></div><div><span>✓</span><p><b>当前房源合同已匹配</b><small>合同地址、租期与这套房源的信息一致。</small></p></div><div><span>¥</span><p><b>租金记录已提交</b><small>仅展示验证结果，不公开原始支付记录。</small></p></div></div></section><section className="zuji-safe-box"><b>交易安全提醒</b><p>看房前不要支付押金、定金或转账。请勿点击陌生链接，优先在站内沟通。</p></section></article><aside className="zuji-contact-card"><span>月租金</span><strong>¥{listing.price.toLocaleString()} <em>/月</em></strong><div className="zuji-contact-availability"><span>最早入住</span><b>{listing.availableFrom}</b></div><p><i /> 发布者通常会在 24 小时内回复</p><button className="primary" onClick={() => openAction("viewing")}>预约看房</button><button className="secondary" onClick={() => openAction("message")}>先问问发布者</button><small>发送后可继续在站内沟通，无需先交换微信</small></aside></section>{action && <ActionDialog action={action} message={message} setMessage={setMessage} date={date} setDate={setDate} time={time} setTime={setTime} note={note} setNote={setNote} result={result} submitting={submitting} today={today} onClose={() => setAction(null)} onMessage={submitMessage} onViewing={submitViewing} onLogin={login} />}<footer className="zuji-footer"><div className="zuji-container"><a className="zuji-brand" href="/"><span>租</span><b>租迹 <em>ZUJI</em></b></a><p>让转租回到租客之间。</p><small>© 2026 ZUJI</small></div></footer></main>;
}
