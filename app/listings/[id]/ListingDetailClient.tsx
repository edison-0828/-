...
export default function ListingDetailClient({ id }: { id: string }) {
  ...
  useEffect(() => {
    fetch(`/api/listings/${encodeURIComponent(id)}`).then(async (response) => {
      ...
      if (!getClientCache<ApiListing>(`listing:${id}`)) setError(reason instanceof Error ? reason.message : "房源加载失败");
  ...
  if (loading) return <main className="zuji-page zuji-black-yellow-theme"><div className="zuji-container zuji-detail-loading">正在加载房源…</div></main>;
  if (!listing || error) return <main className="zuji-page zuji-black-yellow-theme"><div className="zuji-container zuji-detail-loading"><b>{error || "房源不存在"}</b><Link href="/">返回房源列表</Link></div></main>;
